...
import FileUploader from './FileUploader';  // Импортируем компонент загрузки файла
  ...
  return (
    <div>
      ...
      <div className='menu'>
          ...
          {userID !== -1 ? (
            <div>
              ...
              <div style={{ paddingTop: '0.5rem' }}>
                <Button className='button' onClick={handleCreateVertexButton} type='text' block>
                  Create vertex
                </Button>
              </div>
            </div>
        ...
        <div style={{ paddingTop: '0.5rem' }}>
          <Button className='button' onClick={handleCreateEdgeButton} type='text' block>
            Create edge
          </Button>
        </div>
        <div style={{ paddingTop: '0.5rem' }}>
          <Button className='button' onClick={handleRemoveVertexButton} type='text' block>
            Remove vertex
          </Button>
        </div>
        <div style={{ paddingTop: '0.5rem' }}>
          <Button className='button' onClick={handleRemoveEdgeButton} type='text' block>
            Remove edge
          </Button>
        </div>
        <div style={{ paddingTop: '0.5rem' }}>
        ...
        {props.isDirected && props.isWeighted && (
          <div>
            <div style={{ paddingTop: '0.5rem', paddingLeft: '1.7rem' }}>
              <Input
                type='text'
                value={source}
                onChange={e => setFlowResearch(e.target.value, true)}
                style={{ width: '94px' }}
                placeholder='source'
              />
            </div>
            <div style={{ paddingTop: '0.5rem', paddingLeft: '1.7rem' }}>
              <Input
                type='text'
                value={sink}
                onChange={e => setFlowResearch(e.target.value, false)}
                style={{ width: '94px' }}
                placeholder='target'
              />
            </div>
        ...
        <div style={{ paddingTop: '0.5rem', paddingLeft: '1.7rem' }}>
          <div>
            <label>
              <Checkbox onChange={handleIsWeightedCheckbox}> Weighted </Checkbox>
            </label>
          </div>
        </div>
        <div style={{ paddingTop: '0.5rem' }}>
        ...
        {userID === -1 && (
          <div style={{ paddingTop: '0.5rem' }}>
            <Button className='button' onClick={goToSignInPage} type='text' block>
              Sign In
            </Button>
          </div>
        )}
        {userID !== -1 && (
          <div style={{ paddingTop: '0.5rem' }}>
            <Button className='button' onClick={() => setIsGraphSaveMode(!isGraphSaveMode)} type='text' block>
              Save Graph
            </Button>
            <div style={{ paddingTop: '0.5rem' }}>
              <Button
                ...
                type='text'
                block
                onClick={() => setAddCommentMode(!isAddCommentMode)}
              >
                Add comment
              </Button>
            </div>
